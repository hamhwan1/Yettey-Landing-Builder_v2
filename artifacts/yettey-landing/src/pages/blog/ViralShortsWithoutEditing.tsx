    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative pt-32 pb-10 overflow-hidden">
        ...
        <div className="relative max-w-3xl mx-auto px-4">
          ...
          >
            <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4" /> March 22, 2026</span>
            <span className="flex items-center gap-1.5"><Clock className="w-4 h-4" /> 7 min read</span>
            <div className="flex items-center gap-2 flex-wrap">
              <Tag className="w-4 h-4" />
              {["Short Form Video Strategy", "Content Creation Workflow", "AI Tools for Creators"].map((tag) => (
                ...
                {["Hook", "Body", "CTA"].map((step, i) => (
                  <div key={step} className="flex items-center gap-4 sm:gap-6">
                    <div
                      ...
                      style={{ backgroundColor: `${ACCENT}${i === 0 ? "25" : "15"}`, border: i === 0 ? `2px solid ${ACCENT}60` : `1px solid ${ACCENT}20` }}
                    >
                      <span className="text-[11px] font-mono font-bold text-muted-foreground">{`0${i + 1}`}</span>
                      <span className="font-bold text-lg text-foreground">{step}</span>
                      <span className="text-[10px] text-muted-foreground">{i === 0 ? "0–2s" : i === 1 ? "2–8s" : "End"}</span>
                    </div>
